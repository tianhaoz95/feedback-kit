...
export async function selectReleaseCandidates(
  ...
  return ((data ?? []) as FeedbackItem[]).map((item): ReleaseCandidate => {
    ...
    if (git(["cat-file", "-e", `${item.fix_commit_sha}^{commit}`], options.cwd) === null) {
      return { item, included: false, reason: `fix commit ${item.fix_commit_sha.slice(0, 7)} not found locally — git fetch, or pass --include` };
    }
...
export async function recordRelease(
  client: SupabaseClient,
  input: { projectId: string; build: string; version?: string; commitSha?: string | null; productKey?: string; feedbackIds: string[] },
): Promise<string[]> {
...
4. When you open a pull request, put \`FeedbackKit: ${item.id}\` in its description — FeedbackKit tracks the PR from there. If you commit without a PR, call \`link_fix\` with the commit sha instead.
