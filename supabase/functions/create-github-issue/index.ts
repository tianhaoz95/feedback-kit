    ...
    return json(
      {
        ...
        message: "This project has no GitHub repository connected. Connect one in the GitHub tab.",
      },
      400
    );
  }
  ...
  if (feedback.screenshot_annotated_path) {
    try {
      ...
      if (!dlErr && fileData) {
        ...
        const repoAssetUrl = await uploadScreenshotToRepo(token, owner, repo, feedback.id, bytes);
        if (repoAssetUrl) {
...
*Logged via [FeedbackKit](https://tianhaoz95.github.io/feedback-kit/) from report \`${feedback.id}\`*`;
  ...
  try {
    issue = await createGitHubIssue(token, owner, repo, title, issueBody);
  } catch (err) {
